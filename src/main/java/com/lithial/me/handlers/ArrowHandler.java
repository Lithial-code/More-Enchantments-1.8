   ...
   // }
    ...
    public boolean trySetTrackingArrow(EntityArrow newArrow, EntityPlayer player) {
        if ((canArrowBeTracked(trackingArrow, player) == false || newArrow.ticksExisted < trackingArrow.ticksExisted) && canArrowBeTracked(newArrow, player)) {
            ...
            activeTrackingArrowData = new ArrowEntityTracker(newArrow, new Vec3(0, 0, 0), null);
            return true;
        }
        return false;
    }
    ...
    public void manageArrowTracking(boolean homeIn, EntityPlayer player) {
        ...
        Vec3 end = new Vec3(start.xCoord + lookVector.xCoord * trackingArrow_maxDist * 1000, start.yCoord + lookVector.yCoord * trackingArrow_maxDist * 1000, start.zCoord + lookVector.zCoord * trackingArrow_maxDist * 1000);
        Vec3 arrowPos = new Vec3(trackingArrow.posX, trackingArrow.posY, trackingArrow.posZ);
        double speed = 1.5f;
    }
}*/
        /*////////////////////
        ...
        if (m != null) {
            ...
            target.xCoord = m.getBlockPos().getX() + 0.5;
            target.yCoord = m.getBlockPos().getY() + 0.5;
            target.zCoord = m.getBlockPos().getZ() + 0.5;
        }
        Vec3 vec = new Vec3(0, 0, 0);
        start = new Vec3(player.posX, player.posY + player.getEyeHeight(), player.posZ);
        end = new Vec3(start.xCoord + lookVector.xCoord * trackingArrow_maxDist, start.yCoord + lookVector.yCoord * trackingArrow_maxDist, start.zCoord + lookVector.zCoord * trackingArrow_maxDist);
    ...
    public static MovingObjectPosition rayTraceBlocks_do_do(Vec3 par1Vec3D, Vec3 par2Vec3D, boolean par3, boolean par4, World world, int allowableBlocksType) {
        ...
        Block k1 = world.getBlockState(pos).getBlock();
        //int i2 = world.getBlockState(pos).getBlock().getMetaFromState(state).getBlockMetadata(l, i1, j1);
       // Block block = Block.blocksList[k1];
        if ((!par4 || k1 == null || k1.getCollisionBoundingBox(world, pos, state)!= null) && k1!= Block.getBlockById(0) && isBlockAllowable(k1, i2, par3, allowableBlocksType)) {
        ...
        for (int l1 = 200; l1-- >= 0;) {
            ...
            // was createVector
            Vec3 vec3d = new Vec3(par1Vec3D.xCoord, par1Vec3D.yCoord, par1Vec3D.zCoord);
