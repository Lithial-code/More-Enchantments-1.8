...
public class OpenGlHelper {
	...
	public static Vec3 closestLinePartToPoint(Vec3 start, Vec3 end, Vec3 point) {
		if (end == null || start == null || point == null)
			return null;
		Vec3 u = new Vec3(end.xCoord - start.xCoord, end.yCoord - start.yCoord, end.zCoord - start.zCoord);
		Vec3 v = new Vec3(point.xCoord - start.xCoord, point.yCoord - start.yCoord, point.zCoord - start.zCoord);
		u = u.normalize();
		double s = u.dotProduct(v);
		return new Vec3(start.xCoord + u.xCoord * s, start.yCoord + u.yCoord * s, start.zCoord + u.zCoord * s);
	}
	...
	public static Vec3 lineIntersectPlane(Vec3 line_p1, Vec3 line_p2, Vec3 plane_p1, Vec3 plane_p2, Vec3 plane_p3, boolean inLimits) {
		Vec3 lineDir = new Vec3(line_p2.xCoord - line_p1.xCoord, line_p2.yCoord - line_p1.yCoord, line_p2.zCoord - line_p1.zCoord);
		Vec3 planeNormal = calculatePlaneNormal(plane_p1, plane_p2, plane_p3);
		Vec3 diffPoint = new Vec3(plane_p1.xCoord - line_p1.xCoord, plane_p1.yCoord - line_p1.yCoord, plane_p1.zCoord - line_p1.zCoord);
		...
		return new Vec3(line_p1.xCoord + (line_p2.xCoord - line_p1.xCoord) * dist, line_p1.yCoord + (line_p2.yCoord - line_p1.yCoord) * dist, line_p1.zCoord + (line_p2.zCoord - line_p1.zCoord) * dist);
	}
	...
	public static Vec3 lineIntersectionEntity(Vec3 line_p1, Vec3 line_p2, Entity entity) {
		...
		AxisAlignedBB cb = entity.getBoundingBox();
		// front
		Vec3 side1 = new Vec3(cb.maxX, cb.maxY, cb.minZ);
		Vec3 side2 = new Vec3(cb.maxX, cb.minY, cb.minZ);
		Vec3 side3 = new Vec3(cb.minX, cb.minY, cb.minZ);
		Vec3 side4 = new Vec3(cb.minX, cb.maxY, cb.minZ);
		Vec3 result = lineIntersectsQuad(line_p1, line_p2, side1, side2, side3, side4);
		if (result != null)
			return result;

		// Left
		side1 = new Vec3(cb.minX, cb.maxY, cb.maxZ);
		side2 = new Vec3(cb.minX, cb.minY, cb.maxZ);
		side3 = new Vec3(cb.minX, cb.minY, cb.minZ);
		side4 = new Vec3(cb.minX, cb.maxY, cb.minZ);
		result = lineIntersectsQuad(line_p1, line_p2, side1, side2, side3, side4);
		if (result != null)
			return result;
		// Back
		side1 = new Vec3(cb.maxX, cb.maxY, cb.maxZ);
		side2 = new Vec3(cb.maxX, cb.minY, cb.maxZ);
		side3 = new Vec3(cb.minX, cb.minY, cb.maxZ);
		side4 = new Vec3(cb.minX, cb.maxY, cb.maxZ);
		...
		side1 = new Vec3(cb.maxX, cb.maxY, cb.maxZ);
		side2 = new Vec3(cb.maxX, cb.minY, cb.maxZ);
		side3 = new Vec3(cb.maxX, cb.minY, cb.minZ);
		side4 = new Vec3(cb.maxX, cb.maxY, cb.minZ);
		result = lineIntersectsQuad(line_p1, line_p2, side1, side2, side3, side4);
		if (result != null)
			return result;
		// Top
		side1 = new Vec3(cb.maxX, cb.maxY, cb.maxZ);
		side2 = new Vec3(cb.maxX, cb.maxY, cb.minZ);
		side3 = new Vec3(cb.minX, cb.maxY, cb.minZ);
		side4 = new Vec3(cb.minX, cb.maxY, cb.maxZ);
		result = lineIntersectsQuad(line_p1, line_p2, side1, side2, side3, side4);
		if (result != null)
			return result;
	...
	public double approxDistAlongLine(Vec3 start, Vec3 end, Vec3 point) {
	 ...
	 public Vec3 screenCoordsFrom3D(float x, float y, float z) {
		...
		try {
			viewport = (IntBuffer) ReflectionHelper.getPrivateValue(ActiveRenderInfo.class, new ActiveRenderInfo(), "viewport");
			modelview = (FloatBuffer) ReflectionHelper.getPrivateValue(ActiveRenderInfo.class, new ActiveRenderInfo(), "modelview");
			projection = (FloatBuffer) ReflectionHelper.getPrivateValue(ActiveRenderInfo.class, new ActiveRenderInfo(), "projection");
		} catch (Exception e) {
