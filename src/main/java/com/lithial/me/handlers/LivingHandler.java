...
/*		@SubscribeEvent
	public void Teleport(LivingUpdateEvent event) {
		if ((event.entity instanceof EntityPlayer)) {
			EntityPlayer player = (EntityPlayer) event.entityLiving;


			if(Utils.getEnchHelp(player, Enchantments.sunder.effectId, player.getCurrentArmor(2))!= 0){
				if(KeyBind.sneaking){
				 	double posx = player.posX*30D;
					double posy = player.posY;
					double posz =  player.posZ*30D;
					if(posy <1)
						posy = 1;
 					player.setPosition(posx,posy+1,posz);
